...
package object util {
  val NakedZero = BigDecimal(new java.math.BigDecimal(0))
  val Zero = Option(NakedZero)
  implicit def enrichBigDecimal(bd: BigDecimal): RichBigDecimal = RichBigDecimal(Option(bd))
  implicit def enrichOptBigDecimal(obd: Option[BigDecimal]): RichBigDecimal = RichBigDecimal(obd)
